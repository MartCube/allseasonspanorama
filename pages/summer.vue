...
export default {
	async fetch() {
		const page = await this.$prismic.api.getSingle('summer')
		const activities = await this.$prismic.api.query([this.$prismic.predicates.at('document.type', 'activity'), this.$prismic.predicates.at('document.tags', ['summer'])])
